...
def home_page():
    if session.get('user'):
        return render_template("pages/home.html", config = settings.config, user = session["user"])
    return render_template("pages/home.html", config = settings.config, user = {})
        ...
        if request.method == 'POST':
            ...
            if query_response["USER_EXISTS"]:
                if encrypt.check_password(dict_payload["USER_PASS"], query_response["USER_PASS"]):
                    ...
                    return render_template("pages/signin.html", config = settings.config, user = {}, error_message="Incorrect password. Please Try Again!")
            else:
                return render_template("pages/signin.html", config = settings.config, user = {}, error_message="No Email Found! Try Again or Sign Up!")
        return render_template("pages/signin.html", config = settings.config, user = {})
    ...
    if session.get('user'):
        ...
        if request.method == 'POST':
            ...
            if query_response["INSERT"] == "SUCCESS":
                ...
                return render_template("pages/signup.html", config = settings.config, user = {}, error_message=query_response["MESSAGE"])
            else:
                return render_template("pages/signup.html", config = settings.config, user = {}, error_message="Unknown error. Please try again!")

        return render_template("pages/signup.html", config = settings.config, user = {})
...
def account_profile():
    if session.get('user'):
        ...
        api_return = api.post_get_content(settings.picture_api_url, f"get_picture/profile_picture", dict_payload)
        if api_return:
        ...
        return render_template("pages/account/profile/profile.html", config = settings.config, user = user_data, default_image=default_image, encoded_image=encoded_image)
    else:
...
def account_profile_edit_picture():
    if session.get('user'):
        ...
        if request.method == 'GET':
            ...
            return render_template("pages/account/profile/edit/picture.html", user = session.get('user'), default_image=default_image, encoded_image=encoded_image)
    else:
        return redirect(url_for('signin_page'))
...
def account_profile_edit_description():
    if session.get('user'):
        ...
        if request.method == 'GET':
            ...
            return render_template("pages/account/profile/edit/description.html", user = session.get('user'), default_image=default_image, encoded_image=encoded_image)
    else:
        return redirect(url_for('signin_page'))
...
def account_hobbies():
    if session.get('user'):
        ...
        return render_template("pages/account/hobbies/hobbies.html", user = session.get('user'), hobbies=hobby_data, default_image=default_image, encoded_image=encoded_image)
    else:
        return redirect(url_for('signin_page'))
...
def view_hobby(hobby_id):
    if session.get('user'):
        ...
        return render_template('pages/account/hobbies/hobby/view.html', user = session.get('user'), hobby=user_data["DATA"][0], picture_urls=picture_urls)
    else:
        return redirect(url_for('signin_page'))
...
def add_hobby():
    if session.get('user'):
        if request.method == 'POST':
            ...
            return render_template('pages/account/hobbies/hobby/add.html',user = session.get('user'))
    else:
        return redirect(url_for('signin_page'))
...
def edit_hobby(hobby_id):
    ...
    if session.get('user'):
        ...
        return render_template('pages/account/hobbies/hobby/edit.html', user = session.get('user'), hobby=user_data["DATA"][0])
    else:
        return redirect(url_for('signin_page'))
...
def account_sessions():
    if session.get('user'):
        ...
        sessions_data = api.post(settings.db_api_url, f"/user_sessions/get_sessions", session.get('user'))["DATA"]
...
def catalog_hobby(hobby_id):
    if session.get('user'):
        ...
        hobby_data = api.post(settings.db_api_url, f"/user_hobbies/get_hobby", request_dict)['DATA'][0]
